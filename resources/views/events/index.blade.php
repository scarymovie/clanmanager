                    ...
                    <div class="relative max-w-xs">
                        ...
                        <label>
                            <input datepicker datepicker-format="dd.mm.yyyy" type="text"
                                   ...
                                   placeholder="{{ date('d.m.Y', strtotime($week)) ?? now()->format('d.m.Y') }}">
                        </label>
                    </div>
                </div>
                ...
                @foreach($events as $event)
                    @if($event->is_hidden === false)
                        ...
                        @elseif(isset($diffWeeks) && $diffWeeks > 0)
                            @forelse($event->status as $status)
                                @if($status->status === 'accept')
                                    <div
                                        ...
                                        <div
                                            ...
                                            <a href="{{ route('events.status', [$clan, $event, $diffWeeks, 'status' => 'decline']) }}">
                                                Не приду
                                            </a>
                                        </div>
                                    </div>
                                @elseif($status->status === 'decline')
                                    <div
                                        ...
                                        <div
                                            ...
                                            <a href="{{ route('events.status', [$clan, $event, $diffWeeks, 'status' => 'accept']) }}">
                                                Приду
                                            </a>
                                        </div>
                                    </div>
                        ...
                        @else
                            @php
                                $current_event_date = \Carbon\Carbon::parse($event->start_date)->addWeek($week ?? 0);
                            @endphp
                            @forelse($event->status as $status)
                                @if(\Carbon\Carbon::parse($status->event_date)->diffInWeeks($current_event_date, false) === 0)
                                    @if($status->status === 'accept')
                                        <div
                                            class="mb-4 block max-w-xs p-6 bg-green-300 border border-gray-200 rounded-lg shadow-md
                                        hover:bg-green-300 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 event">
