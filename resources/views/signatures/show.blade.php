                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @foreach($members as $member)
                            @if(isset($member->guildWarMemberStatuses->first()->title) && $member->guildWarMemberStatuses->first()->title === 'accept')
                                <tr class="border-b dark:bg-gray-900 dark:border-gray-700 bg-green-300" style="background-color: #75E175">
                            @elseif(isset($member->guildWarMemberStatuses->first()->title) && $member->guildWarMemberStatuses->first()->title === 'decline')
                                <tr class="border-b dark:bg-gray-900 dark:border-gray-700 bg-red-300" style="background-color: #FB6767">
                            @elseif(isset($member->guildWarMemberStatuses->first()->title) && $member->guildWarMemberStatuses->first()->title === 'confirmed')
                            ...
                            @endif
                                    ...
                                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ isset($member->guildWarMemberStatuses->first()->title) ? $member->guildWarMemberStatuses->first()->title : 'Нет информации' }}
                                    </td>
                                    <td>
                                        ...
                                        @elseif(isset($member->guildWarMemberStatuses->first()->title) && $member->guildWarMemberStatuses->first()->title === 'rejected')
                                            <a href="{{ route('gvg.masterUpdate', [$clan, $guildWar, $member, 'status' => 'confirmed']) }}"
